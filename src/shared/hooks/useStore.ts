...
import { TGlobalActions, TGlobalInitialState, vanillaStore } from "../store/store";
...
export function useSliceState<T, K extends keyof T>(stateName: keyof T) {
  const sliceState = useStore(vanillaStore, (state) => state[stateName as keyof TGlobalInitialState]);
